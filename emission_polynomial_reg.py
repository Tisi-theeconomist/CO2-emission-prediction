...
plt.tight_layout()
seabornInstance.distplot(dataset['carbon'])
...
X_poly = poly_reg.fit_transform(X[:, 0:11]) #select non categorical to transfrom
X_poly = np.hstack((X_poly, X[:, 12:])) #merge the categorical variables to X_poly
...
#Compare actual test set and predicted values
...
plt.show() #perform much better than linear regression
